...
namespace CoffeeShop.Models.Services
{
    public class ShoppingCartRepository : IShoppingCartRepository
    {
        ...
        public int UpdateCartItemQuantity(int productId, int quantity)
        {
            var shoppingCartItem = dbContext.ShoppingCartItems.FirstOrDefault(s => s.Product.Id == productId && s.ShoppingCartId == ShoppingCartId);
            
            if (shoppingCartItem != null)
            {
                if (quantity > 0)
                {
                    shoppingCartItem.Qty = quantity;
                    dbContext.SaveChanges();
                    return quantity;
                }
                else if (quantity <= 0)
